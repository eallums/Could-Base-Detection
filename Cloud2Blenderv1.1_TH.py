...
if args.endtime is None:
  if args.starttime is None:
    args.starttime = 0
    args.endtime = time.size  
  else:
    args.endtime = args.starttime + 1
    
print("Dataset   : ", args.dataset)
print("Device    : ", args.device)
print("Run type  : ", args.type)
print("Time range: ", args.starttime, "-", args.endtime)
print("X location: ", args.xloc)
...
nwidth = int(width/dx)
if(args.xloc):
  icameras = [np.int(args.xloc//dx)]
  ncameras = 1
  print(icameras,flush=True)
else:
...
for t in range(args.starttime,args.endtime):
  ...
  #f.close()
  #del ql
  print('-----Placing Cloud Particles-----',flush=True)

  for icam in (icameras):
    print ("Camera at x =", icam*dx)
    if(args.type == "masked"):
      filename = render_landing+str(time[t])+'/TSI_R_MASKED (along x = '+'{:.0f}'.format(icam * dx)+').avi' # setting save location
    elif(args.type == "pretty"):
      filename = render_landing+str(time[t])+'/TSI_R (along x = '+'{:.0f}'.format(icam * dx)+').avi' # setting save location
      
    bpy.data.scenes['Scene'].render.filepath = filename
    print("icam",icam,nwidth,flush=True)
    
    if(os.path.isfile(filename)):
      print("Skipping ",filename," exists.",flush=True)
      continue
    else:
...
#        iBU = ((i-icam) / x.size - 0.5) * 16 + 2 # scaling to bring things into blender space from simulation space
    ...
    '''
    print('----- Assigning materials and volumetric Densities -----',flush=True)
